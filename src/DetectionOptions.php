...
class DetectionOptions {
    public $wp2static_detection_options = [];
    public $wp2static_option_key = null;
    public $wp2static_detection_options_keys = [
        'detectArchives',
        'detectAttachments',
        ...
        // 'additionalUrls'    v
        // 'excludeURLs' TODO: --> into own addons to run at end of detect hook 
    ];
    ...
    public function __construct( string $option_key ) {
        $this->wp2static_detection_options_keys = apply_filters(
            'wp2static_add_option_keys',
            $this->wp2static_detection_options_keys
        );
        ...
        $this->wp2static_detection_options = $options;
    ...
    public function __set( string $name, $value ) : Options {
        $this->wp2static_detection_options[ $name ] = $value;

        if ( ! array_key_exists( $name, $this->wp2static_detection_options ) ) {
        ...
        if ( empty( $value ) ) {
            unset( $this->wp2static_detection_options[ $name ] );
        }
    ...
    public function __get( string $name ) {
        $value = array_key_exists( $name, $this->wp2static_detection_options ) ?
            $this->wp2static_detection_options[ $name ] : null;
    ...
    public function getAllOptions(
        ...
        foreach ( $this->wp2static_detection_options_keys as $key ) {
    ...
    public function optionExists( string $name ) : bool {
        return in_array( $name, $this->wp2static_detection_options_keys );
    }

    public function save() : bool {
        return update_option(
            $this->wp2static_option_key,
            $this->wp2static_detection_options
        );
    }
    ...
    public function saveAllOptions() : void {
        ...
        foreach ( $this->wp2static_detection_options_keys as $option ) {
