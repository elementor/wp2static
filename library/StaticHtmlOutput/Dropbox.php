...
class StaticHtmlOutput_Dropbox {
    ...
    /**
     * Create DropBox deployment list
     *
     ...
     * @return void
     */
    public function create_dropbox_deployment_list(
        $dir,
        $archiveName,
        $remotePath
    ) {
        ...
        foreach ( $files as $item ) {
            if ( $item != '.' && $item != '..' && $item != '.git' ) {
                if ( is_dir( $dir . '/' . $item ) ) {
                    $this->create_dropbox_deployment_list(
    ...
    public function get_item_to_export() {
        ...
        // TODO reduce the 2 file reads here, this just trimming the first line
    ...
    public function transfer_files( $viaCLI ) {
        ...
        // vendor specific - dropbox api url
