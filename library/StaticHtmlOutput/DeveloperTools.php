...
/**
 * URL: http://php.net/manual/en/function.microtime.php
 *
 * Example usage:
 *
 * unset($t);
 ...
 * Get benchmark
 *
 * @param array   $arg_t  Timestamp
 * @param boolean $arg_ra Append "ms" flag
 * @return mixed
 */
function mini_bench_to( $arg_t, $arg_ra = false ) {
    ...
    foreach ( $arg_t as $cle => $val ) {
        if ( $cle != 'start' ) {
            $prcnt_t = round( ( $val - $prv_val ) * 1000, 4 ) / $tttime;
            $prcnt_t = round( ( $prcnt_t * 100 ), 1 );
            if ( $arg_ra ) {
