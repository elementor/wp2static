...
class StaticHtmlOutput_GitHub {
     ...
     * Constructor
     *
     * @param string $userRepository Repository
     * @param string $accessToken    Access token
     * @param string $branch         Branch
     * @param string $remotePath     Remote path
     * @param string $uploadsPath    Upload path
     */
    public function __construct(
        ...
        list( $this->_user, $this->_repository )
            = explode( '/', $userRepository );
        $this->_accessToken = $accessToken;
        $this->_branch = $branch;
        $this->_remotePath = $remotePath;
        $this->_exportFileList = $uploadsPath .
            '/WP-STATIC-EXPORT-GITHUB-FILES-TO-EXPORT';
        $this->_globHashAndPathList = $uploadsPath .
     ...
     * @return void
     */
    public function create_github_deployment_list(
        $dir,
        $archiveName,
        $remotePath
    ) {
        /**
         * TODO: move this into a parent class as identical to bunny and
    ...
    public function upload_blobs( $viaCLI = false ) {
        ...
        try {
            ...
            WsLog::l(
                'GITHUB: Error creating blob (API limits exceeded?):' . $e
            );
            error_log(
                'error creating blog in GitHub (API limits exceeded?)'
            );
            // TODO: show current rate limits
            // TODO: ... https://developer.github.com/v3/rate_limit/
            $coreLimit = $client->api( 'rate_limit' )->getCoreLimit();
            error_log( $coreLimit );
        }//end try

        $globHashPathLine = $globHash['sha'] . ',' . rtrim( $targetPath ) .
    ...
    public function commit_new_tree() {
        ...
        $commitData = array(
            'message' => 'WP Static HTML Output plugin: ' .
                date( 'Y-m-d h:i:s' ),
            'tree' => $newTree['sha'],
