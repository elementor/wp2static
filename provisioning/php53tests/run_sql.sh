...
sudo docker build -t leonstafford/wordpress-static-html-plugin-php53:latest . 
# this container is simpler, exits quicker, so need to give it the loop to stay alive..
sudo docker run --name plugindev53 --net php53net --link php53sql:mysql -p 8093:80 --ip 172.19.0.6 -d -v $(pwd):/app leonstafford/wordpress-static-html-plugin-php53 tail -f /dev/null
