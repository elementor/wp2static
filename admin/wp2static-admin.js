...
jQuery(($) => {
 ...
 Export Log in case of more info.`;
    ...
    function prepareInitialFileList() {
        ...
        sendWP2StaticAJAX("generate_filelist_preview", generateFileListSuccessCallback, generateFileListFailCallback);
    }
 ...
 <a id="downloadExportLogButton" href="#">try again</a>`);
    ...
    function downloadExportLog() {
        ...
        sendWP2StaticAJAX("download_export_log", downloadExportLogSuccessCallback, downloadExportLogFailCallback);
    }
    $(document).on("click", "#detectEverythingButton", (evt) => {
        evt.preventDefault();
        $('#detectionOptionsTable input[type="checkbox"]').attr("checked", 1);
    });
    $(document).on("click", "#deleteCrawlCache", (evt) => {
        evt.preventDefault();
        $("#current_action").html("Deleting Crawl Cache...");
        sendWP2StaticAJAX("delete_crawl_cache", deleteCrawlCacheSuccessCallback, deleteCrawlCacheFailCallback);
    });
    $(document).on("click", "#detectNothingButton", (evt) => {
        evt.preventDefault();
        $('#detectionOptionsTable input[type="checkbox"]').attr("checked", 0);
    });
    $(document).on("click", "#downloadExportLogButton", (evt) => {
        evt.preventDefault();
        downloadExportLog();
    });
    ...
    function updateTimer() {
        ...
        $("#export_timer").html("<b>Export duration: </b>" + millisToMinutesAndSeconds(runningTime));
    }
    function startExport() {
        ...
        sendWP2StaticAJAX("prepare_for_export", startExportSuccessCallback, ajaxErrorHandler);
    }
    ...
    function getValidationErrors() {
        ...
        const requiredFields = deployOptions[currentDeploymentMethod].requiredFields;
        if (requiredFields) {
    ...
    function hideOtherVendorMessages() {
        ...
        $.each(notices, (index, element) => {
            if (!$(element).hasClass("wp2static-notice")) {
                $(element).hide();
            }
        });
    }
    function setFormProcessor(fp) {
    ...
    function populateFormProcessorOptions(fps) {
    ...
    function notifyMe() {
        ...
        if (window.location.protocol === "https:") {
            if (Notification.permission !== "granted") {
                ...
                const notification = new Notification("WP Static HTML Export", {
...
Wordpress_Shiny_Icon.svg/768px-Wordpress_Shiny_Icon.svg.png`,
    ...
    function loadLogFile() {
        ...
        // load the log file
        $.get(logFileUrl + "?cacheBuster=" + Date.now(), (data) => {
            // hide loading icon
    ...
    $(document).on("click", "#GoToDetectionTabButton", (evt) => {
        evt.preventDefault();
        changeTab("URL Detection");
    });
    $(document).on("click", "#GoToDeployTabButton,#GoToDeployTabLink", (evt) => {
        evt.preventDefault();
        changeTab("Deployment");
    });
    // TODO: create action for #GenerateZIPOfflineUse
    // and #GenerateZIPDeployAnywhere
    $(document).on("click", "#GoToAdvancedTabButton", (evt) => {
        evt.preventDefault();
        changeTab("Advanced Options");
    });
    $(document).on("click", ".nav-tab", (evt) => {
        evt.preventDefault();
        changeTab($(evt.currentTarget).text());
    });
    $(document).on("submit", "#general-options", (evt) => {
        evt.preventDefault();
    });
    $(document).on("click", "#send_supportRequest", (evt) => {
        evt.preventDefault();
    ...
    $("#wp2static-footer").on("click", ".resetDefaultSettingsButton", (event) => {
        event.preventDefault();
        sendWP2StaticAJAX("reset_default_settings", resetDefaultSettingsSuccessCallback, resetDefaultSettingsFailCallback);
    });
    $("#wp2static-footer").on("click", ".saveSettingsButton", (event) => {
        event.preventDefault();
        saveOptions();
    });
    ...
    $(".wrap").on("click", "#delete_deploy_cache_button", (event) => {
        event.preventDefault();
        const button = event.currentTarget;
        spinner = $(button).siblings("div.spinner");
        spinner.show();
        sendWP2StaticAJAX("delete_deploy_cache", deleteDeployCacheSuccessCallback, deleteDeployCacheFailCallback);
    });
    ...
    $(".wrap").on("click", '[id$="-test-button"]', (event) => {
        event.preventDefault();
        spinner = $("button").siblings("div.spinner");
        spinner.show();
        sendWP2StaticAJAX("test_" + currentDeploymentMethod, testDeploymentSuccessCallback, testDeploymentFailCallback);
    });
    $(".wrap").on("click", "#save-and-reload", (event) => {
        event.preventDefault();
        saveOptions();
    });
    $(".spinner").hide();
