...
final class BaseHrefProcessorTest extends TestCase{
    ...
    public function testDoesntCreateWhenEmptyBaseHREFGiven() {
        $dom = new DOMDocument('1.0', 'utf-8');
    ...
    public function testDoesntCreateForOfflineMode() {
        $dom = new DOMDocument('1.0', 'utf-8');
    ...
    public function testDoesntCreateWhenNoHeadElementToAttachTo() {
        $dom = new DOMDocument('1.0', 'utf-8');
        ...
        $base_href_processor->dealWithBaseHREFElement(
            $dom, // DOMDocument
            null, // DOMElement $base_element
            null // DOMNode $head_element
        );

        $result = $dom->saveHtml();

        $this->assertEquals(
            '<html><body> <a href="https://somedomain.com/alink">Some anchor text</a></body></html>' . PHP_EOL,
            $result
        );
    }
}
